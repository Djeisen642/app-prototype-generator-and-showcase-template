...
4. **Optionally create an HTML mockup** at `public/prototypes/{id}/mockup.html` — a self-contained HTML file (styles in `<style>`; an inline `<script>` is encouraged, no external `<script src>`). Hold it to the **Interactive Mockup Standard** below. Set `"htmlFile"` in the index entry.
...
```jsonc
{
  ...
  "voice": [
    { "when": "A key moment", "line": "What the app says.", "note": "Why it's written that way." },
  ],
  "competitive": {
